...
export const searchProducts = async (query: string): Promise<SearchProducts> => {
  return (await axiosInstance.get<SearchProducts>('products/search', { params: { query } }))
      .data;
}

export const getProducts = async (filters: ProductFilters | null): Promise<SortedByCategoryProducts[]> => {
  return (await axiosInstance.get<SortedByCategoryProducts[]>('products', {params: filters ? { query: JSON.stringify(filters)} : null }))
      .data;
}
